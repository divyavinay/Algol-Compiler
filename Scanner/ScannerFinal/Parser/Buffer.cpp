...
Program generates an intermediate file with token number and lexeme for the parser. Steps followed by the program
...
private:
	...
	ScannerClass scan;
	string FinalWord;
	int i = 0;						// iterator for getChar
public:
	int lineNumber = 1;
	
	/* *****************************************************************************
	Objects of scanner class and hashtable class are being injected into bufferClass (dependency injection).
	The keywords are loaded into hashTable in ReadInput
...
int main()
{
	...
	ScannerClass scan = ScannerClass();
	BufferClass bufferObj = BufferClass(hashTable, scan);
	inFileScanner.open("InputFile.txt");
	if (inFileScanner.is_open())
	{
		while (getline(inFileScanner, buffer))
		{
			bufferObj.getChar(buffer);
		}

		if (inFileScanner.eof())
			 bufferObj.PrintToScreen(-1, "EOF", "");
	}
	getchar();
	return 0;
}
